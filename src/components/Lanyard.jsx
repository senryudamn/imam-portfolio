...
export default function Lanyard({ position = [0, 0, 20], gravity = [0, -40, 0], fov = 25 }) {
  ...
  return (
    // FIX 1: Mengganti class CSS dengan inline-style agar tidak error Lanyard.css missing
...
function Band({ maxSpeed = 50, minSpeed = 10, isMobile }) {
  ...
  // Mempertahankan fisika halus dari template
  const segmentProps = { type: 'dynamic', canSleep: false, colliders: false, angularDamping: 4, linearDamping: 4 };

  // FIX 2: Membuang kode compositing Canvas yang rumit karena GLB Anda sudah ada fotonya
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && card.current && j3.current) {
      ...
      // FIX 3: Tali dipaksa menempel ke koordinat lubang besi, mencegah bug "tali putus" dari template asli
  ...
  return (
    <group position={[0, 4, 0]}>
      ...
      <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
        ...
        <group
          position={[0, -1.05, 0]} // Penyesuaian presisi tengah agar kartu tidak miring
        ...
        >
          {/* Cukup memanggil scene utuh karena GLB Anda sudah bergambar */}
